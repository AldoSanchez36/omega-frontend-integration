...
export function SensorTimeSeriesChart({
  ...
  useEffect(() => {
    async function load() {
      ...
      addDebugInfo(`Encoded variable: "${encodedVar}"`);
      addDebugInfo(`Token available: ${token ? 'Yes' : 'No'}`);
      addDebugInfo(`Token preview: ${token ? token.substring(0, 20) + '...' : 'None'}`);
      ...
      try {
        ...
        if (!token) {
          const authError = 'No authentication token found. Please log in again.';
        ...
        addDebugInfo(`✅ Token found, proceeding with API calls`);
        
        // Test 1: Endpoint básico de variable con debugging completo
        const basicEndpoint = `/api/mediciones/variable/${encodedVar}`;
        const fullUrl = `${apiBase}${basicEndpoint}`;
        
        addDebugInfo(`📡 Testing basic endpoint: ${fullUrl}`);
        addDebugInfo(`🔑 Using token: ${token.substring(0, 20)}...`);
        ...
        addDebugInfo(`📋 Request headers: ${JSON.stringify(headers)}`);
        
        const basicRes = await fetch(fullUrl, { headers });
        
        addDebugInfo(`📊 Response status: ${basicRes.status} ${basicRes.statusText}`);
        addDebugInfo(`📊 Response headers: ${JSON.stringify(Object.fromEntries(basicRes.headers.entries()))}`);
        
        if (basicRes.ok) {
          const basicResult = await basicRes.json();
          addDebugInfo(`✅ Basic endpoint successful: ${JSON.stringify(basicResult).substring(0, 100)}...`);
          finalData = basicResult.mediciones || basicResult || [];
          addDebugInfo(`📊 Found ${finalData.length} records`);
        } else {
          const errorText = await basicRes.text();
          addDebugInfo(`❌ Basic endpoint failed: ${errorText}`);
          
          // Si falla por autenticación, intentar re-autenticarse
          if (basicRes.status === 401) {
            addDebugInfo(`🔐 Authentication failed, attempting to re-authenticate...`);
            ...
            try {
              ...
              if (loginResponse.ok) {
                ...
                addDebugInfo(`✅ Re-authentication successful`);
                
                // Actualizar token en localStorage
                if (loginResult.token) {
                  localStorage.setItem('Organomex_token', loginResult.token);
                  addDebugInfo(`🔄 Token updated in localStorage`);
                  ...
                  if (retryResponse.ok) {
                    ...
                    addDebugInfo(`✅ Retry successful with new token`);
                    finalData = retryResult.mediciones || retryResult || [];
                  } else {
                    addDebugInfo(`❌ Retry failed: ${retryResponse.status} ${retryResponse.statusText}`);
                  }
                }
              } else {
                addDebugInfo(`❌ Re-authentication failed: ${loginResponse.status}`);
              }
            } catch (authError) {
              addDebugInfo(`❌ Re-authentication error: ${authError}`);
            }
          }
        }
        
        // Si aún no tenemos datos, probar endpoints alternativos
        if (finalData.length === 0) {
          addDebugInfo(`🔄 No data from basic endpoint, trying alternatives...`);
          
          // Test 2: Endpoint con variable-id
          const variableIdEndpoint = `/api/mediciones/variable-id/${encodedVar}`;
          addDebugInfo(`📡 Trying variable-id endpoint: ${apiBase}${variableIdEndpoint}`);
          
          const variableIdRes = await fetch(`${apiBase}${variableIdEndpoint}`, { headers });
          
          if (variableIdRes.ok) {
            const variableIdResult = await variableIdRes.json();
            addDebugInfo(`✅ Variable-id endpoint successful`);
            finalData = variableIdResult.mediciones || variableIdResult || [];
          } else {
            addDebugInfo(`❌ Variable-id endpoint failed: ${variableIdRes.status}`);
          }
        }
        ...
        <CardContent className="p-6">
          <div className="text-center text-red-600">
            ...
            <div className="mt-4 text-xs text-gray-500">
              ...
              <p>Endpoint: {apiBase}/api/mediciones/variable/{encodeURIComponent(variable)}</p>
              {debugInfo.length > 0 && (
